...
Vercel (ホスティング)
    ...
    │    ├─ mac-mini ページにリダイレクトなしでたどり着けるかだけを判定（スクレイピングなし）
    │    ├─ 200 なら OK、リダイレクトなら NG
    │    ├─ OK → /api/notifyを呼ぶ（任意）
    │    └─ NG → 何もしない
    │
    └─── /api/notify
...
**スクレイピングは行わない。** mac-mini 専用ページにリダイレクトなしでたどり着けたかどうかだけを判定する。

```
判定方法

  ① fetch で mac-mini の URL にアクセス（redirect: "manual" でリダイレクトを追わない）
     https://www.apple.com/jp/shop/refurbished/mac/mac-mini
  ② ステータスが 200 → リダイレクトなしでページに到達できた → OK
  ③ ステータスが 301/302/307/308 → リダイレクトされた → NG（mac-mini 専用ページではない）

※ HTML の取得・解析は行わない。自分で内容を確認したい場合はブラウザで該当ページを開く。
```
...
※ 今回は HTML を取得・解析しない（スクレイピングなし）。ステータス確認のみ。
...
Step 3　在庫チェック（約30分）
    └─ mac-mini ページにリダイレクトなしでたどり着けたかだけを判定（スクレイピングなし）
    └─ /api/check-stock を実装
